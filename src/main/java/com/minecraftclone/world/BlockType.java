...
public enum BlockType {
    ...
    // -----------------------------------------------------------------------
    // Tree variants: birch, jungle, and pine each get their own log and leaf
    // type so different biomes look distinct.  The existing WOOD_LOG / LEAVES
    // are treated as oak; CHERRY_LEAVES is cherry (unchanged).
    //
    // Atlas tiles:
    //   276 = birch-log side (pale cream bark)
    //   277 = jungle-log side (warm dark-brown bark)
    ...
    // They grow into full trees on random ticks (see Farming.tickSaplings).
    // Jungle saplings also support 2×2 planting → giant jungle tree.
    //
    // Atlas tiles: 281 oak sapling, 282 birch, 283 jungle, 284 pine.
    // -----------------------------------------------------------------------

    /** Oak sapling — grows into a standard oak tree when placed on dirt/grass. */
    OAK_SAPLING(534, false, true, 281),
    /** Birch sapling — grows into a birch tree (taller, slimmer than oak). */
    BIRCH_SAPLING(535, false, true, 282),
    /**
     * Jungle sapling — alone it grows into a small jungle tree; four arranged in
     * a 2×2 square grow into a large 2×2-trunk jungle tree.
     */
    JUNGLE_SAPLING(536, false, true, 283),
    ...
    /**
     * True for any sapling (oak, birch, jungle, pine).
     * Saplings are cross-shaped, non-solid, and placed on DIRT or GRASS.
     */
    ...
    /**
     * True for any wood log (oak, birch, jungle, or pine).
     * Cherry trees use WOOD_LOG for their trunk.
     */
