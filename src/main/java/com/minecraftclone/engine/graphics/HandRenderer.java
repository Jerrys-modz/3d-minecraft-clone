...
public class HandRenderer {
    ...
    public void render(Shader shader, TextureAtlas atlas, ItemTextures itemTextures,
                       BlockType type, float bobPhase, float animTime, Matrix4f projection) {
        ...
        if (type.isItem) {
            addItemSprite(type, itemTextures, bobX + idleX, bobY + idleY);
        } else {
            addBlockCube(type, atlas, bobX + idleX, bobY + idleY);
        }
    ...
    private void addBlockCube(BlockType block, TextureAtlas atlas, float bobX, float bobY) {
        model.identity()
                .translate(HAND_X + bobX, HAND_Y + bobY, HAND_Z)
                .rotateY(YAW)
                .rotateX(PITCH)
                .scale(BLOCK_HALF);
