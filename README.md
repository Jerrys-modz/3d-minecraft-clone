...
- **First-person player controller**: WASD walking with gravity and AABB-vs-voxel collision resolved per axis, jumping, stamina-gated sprinting (hold `Left-Ctrl`, or double-tap `W` to latch it on until you let go of `W`), and a creative-only flight mode (`F` to toggle; double-tap `W` also takes off, but only `F` lands you - a double-tap that could switch flight off too meant an accidental double-tap while just moving forward would drop you out of the sky).
...
| `F` | Toggle flight (creative only) - double-tap `W` also takes off, but only `F` lands you |
