...
- **Crafting**: a small fixed recipe table (logs → planks → sticks, sand → glass, planks/stone/ingots + sticks → tools, stick + coal → torches, glass + torch → lamps, 8 stone → furnace, 3 stone → 6 stone slabs, 3 planks → 6 planks slabs) - press `C` with the output selected in your hotbar and, if you have the ingredients, it's crafted directly into your inventory. No crafting-grid UI needed.
...
| `C` | Craft the selected item from its recipe, if you have the ingredients - or smelt it, if you're aiming at a furnace |
...
- Craft pickaxes/axes with `C`: 3 of the tool's base material (planks, stone, iron ingot, or diamond gem) + 2 sticks. Swords are lighter - 2 material + 1 stick. Craft sticks from planks first (2 planks → 4 sticks). Iron and diamond are **smelted** from their ore first (see Furnaces below), not used raw. See `Mining.java` for the exact hardness/tier table.
...
A **furnace** (craft with `C`: 8 stone → 1 furnace, then place it like any other block) turns raw ore into the refined ingot/gem you need for iron and diamond tools. Place a furnace, look at it, select the ore in your hotbar, and press `C` - it smelts one ore into its refined form, consuming one **coal ore** as fuel:
...
├── world/                    # Chunk, World (streaming/meshing), BlockType, Mining, ItemEntity
│   └── gen/                   # TerrainGenerator (noise-based world gen)
├── player/                    # Player controller, PlayerStats, Inventory, Crafting, Smelting, MiningController
