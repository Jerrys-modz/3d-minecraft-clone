...
- **Crafting (Minecraft-style)**: a 3×3 crafting grid - press `E` to open it, arrange ingredients (arrows move the cursor, `Space` places/removes the selected hotbar item, `C` crafts), and the output is determined by the shape. **Shaped** recipes match anywhere in the grid and are mirrored (so an axe works with its handle on either side): a log → 4 planks, 2 planks stacked → 4 sticks, a stone ring → furnace, coal over a stick → 4 torches, glass over a torch → lamp, 3 across → 6 slabs, and the pickaxe/axe/sword shapes in wood/stone/iron/diamond. **Shapeless** recipes ignore arrangement and match on the ingredient combination instead - currently 2 sand (anywhere) → 1 glass. Adding a recipe is a one-liner in `Crafting.java` (see its javadoc).
...
- Water is rendered translucent (alpha-blended in a separate pass after opaque blocks), so you see a blue-tinted world through it rather than x-ray holes; lava is opaque. Flowing water/lava spread via a bounded flood-fill recomputed from placed sources each tick (see Flowing fluids) - it pours down and spreads, but doesn't flow upward or merge with the static ocean/cave fills. Leaves are opaque by default but have a **see-through** mode (settings menu) that switches them to an alpha-cutout texture with holes. (Grass/flowers/berry bushes are cross-shaped and alpha-cutout too.)
