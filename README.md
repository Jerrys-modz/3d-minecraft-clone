...
- **Passive mobs**: pigs, cows and sheep wander the surface to make the world feel alive. They spawn on grass near the player (up to a modest cap), idle and stroll around with a tiny wander brain - gravity and AABB-vs-voxel collision like the player's, single-block step-up, and they never walk off cliffs or into walls - bob while walking, and despawn once they're far away. Each kind is a camera-facing billboard with its own procedurally painted pixel-art sprite (self-contained, no image files), batched per kind into one draw call and lit by the same fog/day-night cycle as everything else. Transient like dropped items, so a herd never follows you across the map.
