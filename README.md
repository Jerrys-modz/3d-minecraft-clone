...
- **Procedural terrain generation**: layered Perlin/fBm noise for continents and oceans (a low-frequency channel creates real ocean basins and landmasses at a natural sea level), rolling hills and mountains, a **temperature + moisture biome system** that assigns each column a biome (ocean / beach / plains / forest / desert / savanna / taiga / snowy / tundra / mountain) and drives its surface blocks, trees and ground cover, winding **rivers** carved as wide shallow channels whose banks sit level with the water, 3D-noise cave systems with lava pooling in the deepest pockets, and four depth-gated ore veins (coal, iron, gold, diamond - rarer ones deeper and sparser, mirroring vanilla Minecraft's progression). Water only forms in bodies that connect to a neighbour, so stray 1-2 block puddles never litter the landscape.
- **Biome-varied vegetation**: dense oak forests and tall grass/flowers/berry bushes where it's wet, sparser oak on plains, conical pine trees in cold taiga and on mountain slopes, cacti in deserts, sandy beaches that hug only the actual coastline, gravel/sand ocean floors, and the occasional boulder on the plains.
