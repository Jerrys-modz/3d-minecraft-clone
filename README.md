...
- Water and lava use translucent, lowered surfaces rather than opaque full cubes, while keeping the renderer single-pass. Flowing water/lava spread via a bounded flood-fill recomputed from placed sources each tick (see Flowing fluids) - they pour down, spread across matching static fluid bodies, but don't flow upward or merge with unrelated static fills. Leaves are opaque by default but have a **see-through** mode (settings menu) that switches them to an alpha-cutout texture with holes - cutout discards transparent fragments in the shader, so it still needs no sorting and stays single-pass. (Grass/flowers/berry bushes are cross-shaped and alpha-cutout too.)
