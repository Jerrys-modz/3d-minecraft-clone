...
- Water and lava are rendered as solid (opaque) blocks rather than alpha-blended, keeping the renderer single-pass. Flowing water/lava spread via a bounded flood-fill recomputed from placed sources each tick (see Flowing fluids) - it pours down and spreads, but doesn't flow upward or merge with the static ocean/cave fills. Leaves are opaque by default but have a **see-through** mode (settings menu) that switches them to an alpha-cutout texture with holes - cutout discards transparent fragments in the shader, so it still needs no sorting and stays single-pass. (Grass/flowers/berry bushes are cross-shaped and alpha-cutout too.)
