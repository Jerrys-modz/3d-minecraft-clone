...
- **Furnace & smelting**: a placeable furnace block (8 stone) that smelts raw ore into the refined ingots/gems iron and diamond tools require. Right-click a placed furnace to open its **furnace GUI**: put ore in the top slot, coal in the fuel slot, and it smelts over time (one coal burns for one smelt) with a live flame and progress arrow. See [Furnaces & smelting](#furnaces--smelting).
...
A **furnace** (craft in the crafting grid: a ring of 8 stone → 1 furnace, then place it like any other block) turns raw ore into the refined ingot/gem you need for iron and diamond tools. Right-click a placed furnace to open its **furnace GUI** - three slots over the player inventory: ore in the top (input) slot, **coal ore** in the middle (fuel) slot, and the smelted result in the output slot. With fuel loaded it smelts **over time** like Minecraft's furnace: each coal burns for one smelt (a glowing flame between the input and fuel slots shows the burn left), and the ore converts progressively (an arrow fills toward the output as it cooks). Shift-clicking moves ore into the input, coal into the fuel, and the output back to your inventory automatically:
