...
- **First-person player controller**: WASD walking with gravity and AABB-vs-voxel collision resolved per axis, jumping, stamina-gated sprinting (hold `Left-Ctrl`, or double-tap `W` to latch it on until you let go of `W`), and a creative-only flight mode (`F` to toggle, or double-tap `W` while in creative).
...
| `F` / double-tap `W` (creative) | Toggle flight (creative only) |
