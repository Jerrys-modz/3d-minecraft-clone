...
- **Crafting**: a small fixed recipe table (logs → planks → sticks, sand → glass, planks/stone/ore + sticks → tools, stick + coal → torches, 3 stone → 6 stone slabs, 3 planks → 6 planks slabs) - press `C` with the output selected in your hotbar and, if you have the ingredients, it's crafted directly into your inventory. No crafting-grid UI needed.
...
- A **furnace** and smelting (ore → ingot) ahead of raw-ore tools, plus **stairs/doors/fences/trapdoors** (the mesher now supports partial-cube slabs - see Features - but not stairs' stepped geometry, doors' animated halves, or fences' thin posts).
